...
\begin{example}
    ...
    従って，例 \ref{ex:Borel_algebra} によって $\Rbb\cup\{\pm\infty\}$ は自然に可測空間になる．
    $\Rbb\cup\{\pm\infty\}$ 上の Borel 集合族は
    \begin{align*}
        \Bfrak(\Rbb\cup\{\pm\infty\})
        =\{B\cup S\mid B\in\Bfrak(\Rbb),\ S\in\{+\infty,-\infty\}\}
    \end{align*}
    と特徴づけられる．
\end{example}
...
\begin{example}\label{ex:Lebesgue_outer_measure}
    ...
    \end{align}
    と定めると，これは $\Rbb^n$ 上の外測度になる (\!\cite[p.25]{It63} を参照)．
...
\begin{remark}
    ...
    定義式が対称な形になるように，Lebesgue 内測度を
    \begin{align*}
        \mu_\Lrm^{n,\ast}(E)\stackrel{?}{\defeq}\sup\left\{
            \sum_{i=1}^\infty\mathrm{vol}(I_i)
            \,\middle\vert\,
            \text{$I_1,I_2,\ldots$ は $\Rbb^n$ の区間，$E\supset\bigcup_{i=1}^\infty I_i$}
        \right\}
    \end{align*}
    と定めるとうまくいかない．
    このことは，たとえば $E=[0,1]\cap(\Rbb\setminus\Qbb)$ の場合を考えると了解できる
    (内測度の値は $1$ になってほしいが，うまく内側から区間で近似できない)．
...
\footnote{
    ...
    ただし，定理を素朴に適用して得られるのは $\Ffrak$ が生成する $\sigma$-加法族
    (これは Borel 集合族 $\Bfrak^n$ と一致する) を定義域とする測度であることに注意する．
    特に，後に注意 \ref{rem:Borel_Lebesgue} で見るように $\Bfrak^n\subsetneq\Bfrak_\Lrm^n$ となるので，
    定理によって得られるのは厳密には Lebesgue 測度ではない．
    Lebesgue 測度を得るには，定理の証明において $\Gamma$ を $\Afrak$ 上ではなく $\Mfrak_\Gamma$ 上に制限すればよい．
}．
...
\begin{theorem}[E.\ Hopf の拡張定理，または Carath\'eodory の拡張定理]\label{thm:Hopf_extension}
    ...
    $\Afrak$ を $\Ffrak$ が生成する $\sigma$-加法族とする．

    このとき，測度 $\mu:\Afrak\to\Rbb\cup\{+\infty\}$ で $\mu\vert_\Ffrak=m$ を満たすものが存在するためには，
...
\begin{proof}
    ...
    この $\Gamma$ に対して定理 \ref{thm:outer_measure_to_measure} を適用すれば測度空間 $(X,\Mfrak_\Gamma,\mu)$ を得る．
    $\Ffrak\subset\Mfrak_\Gamma$ \cite[定理 5.2]{It63} で $\Mfrak_\Gamma$ は $\sigma$-加法族なので，
    特に $\Afrak\subset\Mfrak_\Gamma$ であって，$\mu$ を $\Afrak$ 上に制限することで所望の測度が得られる．
...
\begin{theorem}\label{thm:measure_completion}
    ...
    \end{align*}
    と定めると，これは $X$ 上の外測度になる．
    さらに，定理 \ref{thm:outer_measure_to_measure} の完備測度空間
